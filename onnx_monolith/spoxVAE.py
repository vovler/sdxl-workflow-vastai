...
def build_tiled_decoder_onnx_model_with_loop(
    ...
    def tile_decoding_body(iteration_num, _, current_tile_cache):
        ...
        # scalar H: shape=()
        decoded_height_scalar = op.reshape(op.gather(decoded_shape, to_const(np.array([2]))), empty_shape_const, allowzero=1)
        # scalar W: shape=()
        decoded_width_scalar = op.reshape(op.gather(decoded_shape, to_const(np.array([3]))), empty_shape_const, allowzero=1)
    ...
    # grouped_by_row: shape=(num_rows, num_cols, batch, C, row_limit, row_limit)
    ...
    # transposed_for_rows: shape=(num_rows, batch, C, row_limit, num_cols, row_limit)
    ...
    # transposed_for_canvas: shape=(batch, C, num_rows, row_limit, final_row_width)
