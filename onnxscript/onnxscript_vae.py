...
def load_state_dict_from_safetensors(filepath: str) -> Dict[str, np.ndarray]:
    """
    Loads a model's weights and biases from a .safetensors file directly
...
def load_and_create_spox_params(state_dict: Dict[str, np.ndarray]) -> Dict[str, Any]:
    """
    Converts a flat state_dict of NumPy arrays into a nested dictionary 
    of Spox constant Vars.
    """
    spox_params = {}
    for key, value in state_dict.items():
        ...
        for i, part in enumerate(parts[:-1]):
            # Use string keys for indices to create nested dictionaries
...
def spox_resnet_block_2d(
    ...
    # If a shortcut convolution exists in the parameters, use it.
    if "conv_shortcut.weight" in params:
...
def spox_attention_block(
    ...
    # Reshape for multi-head attention style matmul
...
def spox_vae_mid_block(
    ...
    hidden_states = spox_resnet_block_2d(hidden_states, in_channels, in_channels, params["resnets"]["0"], norm_num_groups)
    hidden_states = spox_attention_block(hidden_states, in_channels, params["attentions"]["0"], norm_num_groups)
    hidden_states = spox_resnet_block_2d(hidden_states, in_channels, in_channels, params["resnets"]["1"], norm_num_groups)
...
def spox_down_encoder_block_2d(
    ...
    for i in range(num_layers):
        hidden_states = spox_resnet_block_2d(hidden_states, current_in, out_channels, params["resnets"][str(i)], norm_num_groups)
...
def spox_encoder(
    x: spox.Var, params: Dict[str, spox.Var], config: Dict
) -> spox.Var:
    ...
    if config.get("mid_block_add_attention", True):
        x = spox_vae_mid_block(x, in_channel, params["mid_block"], config["norm_num_groups"])
...
def spox_decoder(
    z: spox.Var, params: Dict[str, spox.Var], config: Dict
) -> spox.Var:
    z = spox_conv_2d(z, params["conv_in.weight"], params["conv_in.bias"], padding=1)

    if config.get("mid_block_add_attention", True):
        z = spox_vae_mid_block(z, config["block_out_channels"][-1], params["mid_block"], config["norm_num_groups"])
...
def spox_autoencoder_kl_forward(
    sample: spox.Var,
    sample_posterior: spox.Var, # Boolean scalar Var
    params: Dict[str, Any],
    config: Dict
) -> spox.Var:
...
if __name__ == '__main__':
    # !!! IMPORTANT !!!
    # Replace this with the actual path to your VAE's .safetensors file.
    SAFETENSORS_FILE_PATH = "/lab/model/vae/diffusion_pytorch_model.safetensors"

    try:
        # 1. Load the state dictionary from the safetensors file
        state_dict = load_state_dict_from_safetensors(SAFETENSORS_FILE_PATH)
        
        # 2. Build the ONNX model using the loaded weights
