...
class SDXLPipeline:
    ...
    def _encode_prompt(self, prompt, is_warmup=False):
        # Tokenize prompt
        ...
        # Get embeddings
        output_l = self.text_encoder_l(input_ids=input_ids_l)
        hidden_states_l = output_l.last_hidden_state
    ...
    def __call__(
        ...
        prompt_embeds, pooled_prompt_embeds = self._encode_prompt(prompt, is_warmup)
        negative_prompt_embeds, negative_pooled_prompt_embeds = self._encode_prompt(negative_prompt, is_warmup)
...
if __name__ == "__main__":
    ...
    # Warmup run
    _ = pipeline(prompt, negative_prompt, is_warmup=True)
    ...
    image = pipeline(prompt, negative_prompt)
    end_time = time.time()
