...
def get_vae_decoder_profiles(bs=1):
    """Get dynamic shape profiles for VAE decoder"""
    # Define a range of latent sizes for dynamic shapes.
    # Common SDXL latent sizes
    min_h, opt_h, max_h = 96, 128, 144  # For 768, 1024, 1152 resolutions
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert ONNX VAE decoder model to TensorRT engine with FP16"
    )
    parser.add_argument(
        "onnx_path",
    ...
    # Get VAE decoder profiles
    input_profiles = get_vae_decoder_profiles(bs=1)
    
    print("Using VAE decoder input profiles:")
    for name, (min_shape, opt_shape, max_shape) in input_profiles.items():
